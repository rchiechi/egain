...
class ColorFormatter(logging.Formatter):
    ...
    def __init__(self):
        super().__init__(fmt='%(asctime)s - %(name)s - [%(levelname)s] %(message)s', datefmt=None, style='%')
    ...
    def format(self, record):
        ...
        if record.levelno == logging.DEBUG:
            self._style._fmt = ColorFormatter.dbg_fmt

        elif record.levelno == logging.INFO:
            self._style._fmt = ColorFormatter.info_fmt

        elif record.levelno == logging.WARNING:
            self._style._fmt = ColorFormatter.wrn_fmt

        elif record.levelno == logging.ERROR:
            self._style._fmt = ColorFormatter.err_fmt
